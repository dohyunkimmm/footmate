...
현재 기준으로 직접 유지하는 문서는 아래 일곱 개입니다.
...
`최종 runtime/수동 QA 확정 → README → Release History/Correction → Case Study → Runbook(절차 변경 시) → Notion 관련 페이지 → 서로 상충하는 pending/미검증 문구 검색 → QA/merge`

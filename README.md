# FootMate v4.5.0 — Matchday Companion

FootMate는 **내 수준에 맞는 풋살 경기를 찾고, 왜 잘 맞는지 이해하고, 조건을 직접 좁힌 뒤 경기 상세에서 참가 판단을 마치고 결제 복구와 경기 당일 운영까지 이어지는 경험**을 설계한 인터랙티브 서비스 기획 프로젝트입니다.

## Current release

- Release: **v4.5.0 · Matchday Operations**
...
- GitHub Actions: **FootMate QA #367 · run 35485268368 · PASS**
- Exact Vercel Production: `072eff6e597c98bb370ece61cba4e74d5975c4a3` · `dpl_DQWXcq8BHG5ZHsudN2YWaEZz8xUY` · **READY** · exact HTTP/Chromium smoke PASS
- Render backup: `072eff6e597c98bb370ece61cba4e74d5975c4a3` · `dep-dankmf2jnfac738vj0u0` · **LIVE** at release verification time
...
10. **State-aware Matchday** — 참가 후 `upcoming → matchday → checked-in` 상태와 late·update·cancel recovery를 분리해 경기 당일 행동을 명확하게 합니다.
11. **Explicit operations boundary** — 위치·지도·팀 채팅·알림 backend는 연결하지 않고 운영 상태·복구 UX만 deterministic simulation으로 검증합니다.
12. **Real / Guided / Evidence separation** — 실제 사용자 화면과 리뷰어 설명·검증 UI를 분리합니다.

## Implemented scope

- Guest-first preferences → recommendation → discovery → decision detail → sign-in → checkout → payment state → confirmation → matchday operations
...
- Separate `footmate:v4:matchday` persistence for post-join operations
- `upcoming → matchday → checked-in` state contract with arrival status
- late → arrival recovery, operational update acknowledgement and cancellation recovery
- actionable Home / Schedule matchday surfaces
- venue/check-in/team-notice UX contract with explicit sample/simulation disclosure
- explicit disclosure that realtime location, map, team chat and notification backend are not connected
...
- 16-section product-first Case Study with Recommendation Core + Discovery & Search + Decision Detail + Join & Payment + Matchday Operations evidence
- Browser E2E + axe and exact Production HTTP/Chromium gates

## Prototype boundary

FootMate v4.5.0은 서비스 기획 검증용 인터랙티브 프로토타입입니다. 추천은 **규칙 기반 explainable ranking + 샘플 데이터 + 브라우저 세션 상태**, 탐색은 deterministic filter/sort와 브라우저/URL persistence로 동작합니다. Decision Detail의 잔여 자리·참가자 구성·시설·운영 정보는 명시된 prototype sample data이며 realtime 운영 데이터가 아닙니다. Join & Payment의 결제 수단·pending·failure·retry·cancel·success, Matchday Operations의 arrival·check-in·late·update·cancel 상태 또한 deterministic simulation입니다. **외부 AI 모델, 회원 DB, 실제 OAuth, 실제 PG 결제, 실시간 수용량, 실시간 참가자 데이터, 실시간 위치·지도, 팀 채팅, 실시간 알림 backend는 연결하지 않았습니다.**
...
- `src/v4/matchday.js` / `matchday.css` — v4.5 Matchday Operations state, arrival/check-in/update/cancel recovery ownership
- `src/v4/case-study-recommendation.js` / `case-study-discovery.js` / `case-study-decision.js` / `case-study-participation.js` / `case-study-matchday.js` — staged Case Study evidence
...
- `tests/e2e/v4-matchday.spec.cjs` — Matchday Operations state / recovery / persistence / mobile / axe gate
- `tests/e2e/v4-case-study.spec.cjs` / `tests/e2e/v4.5-case-study.spec.cjs` — 16-section editorial / Matchday evidence / layout / overflow / axe gates
- `tests/production-v4-smoke.cjs` / `tests/production-v4.5-smoke.cjs` / `tests/e2e/v4-production.spec.cjs` / `tests/e2e/v4.5-production.spec.cjs` — exact Production gates
- `scripts/check-v4-boundary.cjs` / `scripts/check-v4.5-boundary.cjs` — current-tree release and staged boundary checkers
...
문서-only merge로 `main` SHA가 이동하더라도 위 **product/runtime baseline**과 exact Production SHA는 별도로 유지합니다. 각 minor release는 exact Production 검증과 durable release sync를 닫은 뒤 다음 단계로 이동합니다. 다음 단계는 **v4.6 · Return Loop & Reputation**입니다.

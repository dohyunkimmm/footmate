...
-- Keep authenticated read policies single-per-action while allowing operators to inspect drafts and participants.
...
using (
  id = (select auth.uid())
  or exists (select 1 from public.operators o where o.user_id = (select auth.uid()))
);
...
drop policy if exists matches_anon_read on public.matches;
...
drop policy if exists match_slots_anon_read on public.match_slots;
...
to anon
using (
  exists (select 1 from public.matches m where m.id = match_id and m.status <> 'draft')
);
...
begin
  ...
  if v_participation.status = 'canceled' then
    return query select v_participation.id, p_match_id, p_user_id, v_participation.status;
    return;
  end if;
...
revoke execute on function public.operator_cancel_match(uuid) from public, anon;
revoke execute on function public.operator_cancel_participant(uuid,uuid) from public, anon;
...
grant execute on function public.operator_cancel_participant(uuid,uuid) to authenticated;

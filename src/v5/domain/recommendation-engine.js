...
export function distanceScore(minutes){
...
export function levelScore(gap){
  if(gap===0)return 25;
  if(gap===1)return 17;
  if(gap===2)return 8;
  return 0;
...
export function recommendationFor(match,state={}){
  ...
  return Object.freeze({match,score,exactRegion,slotCount,fit:fitLabel(score),spotLabel:slotCount>0?`${state.position} ${slotCount}자리`:match.spot,reasons:Object.freeze(reasons)});
}

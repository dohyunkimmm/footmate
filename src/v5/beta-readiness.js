...
if(root){
  ...
  function recoveryPending(){return sessionStorage.getItem(RECOVERY_KEY)==='1'}
  ...
  function injectNotice(){
    root.querySelector('[data-readiness-notice]')?.remove();
    if(!localNotice)return;
    const shell=root.querySelector('.fm-beta-shell');
    const hero=shell?.querySelector('.fm-beta-hero');
  ...
  function injectRecoveryActions(){
    ...
    form.insertAdjacentHTML('beforeend',`<div data-readiness-auth-actions class="fm-beta-actions" style="justify-content:flex-start;gap:12px;flex-wrap:wrap">
      <button class="fm-beta-link" type="button" data-readiness-action="forgot-password">비밀번호 찾기</button>
      <button class="fm-beta-link" type="button" data-readiness-action="resend-signup">가입 인증메일 다시 보내기</button>
    </div>`);
  }
  ...
  function injectRecoveryPanel(){
    ...
    const shell=root.querySelector('.fm-beta-shell');
    if(!shell||shell.querySelector('[data-readiness-recovery]'))return;
    const hero=shell.querySelector('.fm-beta-hero');
    if(!hero)return;
    hero.insertAdjacentHTML('afterend',`<div class="fm-beta-panel" data-readiness-recovery style="margin-bottom:18px">
      <div class="fm-beta-panel-head"><div><h2>새 비밀번호 설정</h2><p>복구 링크가 확인됐습니다. 새 비밀번호를 설정하면 현재 기기에서 로그인 상태를 이어갑니다.</p></div></div>
      <form class="fm-beta-stack" data-readiness-form="update-password">
        <label class="fm-beta-field"><span>새 비밀번호</span><input name="password" type="password" autocomplete="new-password" minlength="${BETA_SIGNUP_PASSWORD_MIN_LENGTH}" required placeholder="${BETA_SIGNUP_PASSWORD_MIN_LENGTH}자 이상"></label>
        <label class="fm-beta-field"><span>새 비밀번호 확인</span><input name="confirmPassword" type="password" autocomplete="new-password" minlength="${BETA_SIGNUP_PASSWORD_MIN_LENGTH}" required></label>
        <button class="fm-beta-button fm-beta-button--primary" type="submit">비밀번호 변경</button>
      </form>
    </div>`);
  }
  ...
  root.addEventListener('click',async event=>{
    ...
    if(action==='forgot-password'||action==='resend-signup'){
      ...
      try{
        ...
        if(action==='forgot-password'){
          await client.auth.recover({email,redirectTo});
          notice('비밀번호 재설정 이메일을 요청했습니다. 메일의 복구 링크를 확인해주세요.','success');
        }else{
          await client.auth.resendSignup({email,redirectTo});
          notice('가입 인증메일을 다시 요청했습니다. 받은편지함과 스팸함을 확인해주세요.','success');
        }
    ...
    if(action==='check-in'){
      ...
      target.disabled=true;
      try{
        await client.participation.checkIn({accessToken:session.accessToken,matchId:target.dataset.matchId});
        notice('체크인이 완료됐습니다.','success');
    ...
    if(action==='read-notification'){
      ...
      if(!session?.accessToken)return;
      try{
        await client.notifications.markRead({accessToken:session.accessToken,notificationId:target.dataset.notificationId});
        await refreshState({force:true});
  ...
  root.addEventListener('submit',async event=>{
    ...
    try{
      await client.auth.updatePassword({accessToken:session.accessToken,password});
      sessionStorage.removeItem(RECOVERY_KEY);
      history.replaceState(null,'',`${location.pathname}${location.search}`);
      notice('비밀번호를 변경했습니다.','success');
      location.replace('/beta');
    }catch(error){
      notice(String(error?.message||'비밀번호를 변경하지 못했습니다.'),'error');

...
(function(){
  ...
  function patch(){
    ...
    // 13 · Release & Learnings — surface real task-based usability evidence and the project evolution without version labels.
    const release=slides[12];
    setHTML(release,'.fm-next-story h2',lines(
      '사용자 검증과 운영 고도화를 반복하며,',
      '구현 결과와 성과 측정을 구분합니다.'
    ));
    ...
    setHTML(release,'.fm-next-cs-final','<span hidden>Production 기준</span><span>사용성 검증 · 근거와 다음 단계</span>'+rows([
      ['고도화 흐름','PBL 교육 → 과업 기반 사용자 검증 → 제품·운영 상태 고도화 → Production QA → KPI 측정 준비. 각 단계의 근거를 분리해 관리합니다.'],

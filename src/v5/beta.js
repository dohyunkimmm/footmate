...
if(root){
  ...
  function renderAuth(){
    if(!session||!user){
      ...
      return `<div class="fm-beta-panel" id="beta-auth-panel">
        ...
        <form class="fm-beta-stack" data-form="auth">
          ...
          <label class="fm-beta-field"><span>비밀번호</span><input name="password" type="password" autocomplete="${signup?'new-password':'current-password'}" minlength="6" required placeholder="6자 이상"></label>
    ...
    return `<div class="fm-beta-panel">
      ...
      <div class="fm-beta-note" style="margin-top:12px">참가 버튼은 여기서 설정한 포지션을 기준으로 실제 DB 잔여 자리를 확인합니다.</div>
    </div>`;
  }
      ...
      run(async()=>{
        ...
        if(authMode==='signup'){
          ...
          if(result?.access_token){
            writeSession(result);await loadPrivate();await loadMatches();setNotice('가입과 로그인이 완료됐습니다. 프로필을 설정해주세요.','success');
          }else{
          ...
          writeSession(result);await loadPrivate();await loadMatches();setNotice('로그인했습니다.','success');
        }
      });
    }

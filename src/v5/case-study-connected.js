...
(function(){
  ...
  function patch(){
    ...
    const providers=slides[13];if(providers){const lead=providers.querySelector('.fm-next-story-lead');if(lead)lead.textContent='/app의 auth/payment/capacity/notification provider는 deterministic mock을 유지합니다. 반면 /beta는 Supabase Auth·Postgres·RLS·atomic RPC로 실제 회원·경기·포지션 정원·참가/취소를 연결하고, /beta/operator는 allowlist 운영 경로를 사용합니다. 실제 PG·notification delivery·external analytics는 아직 연결하지 않았습니다.';providers.setAttribute('data-v5-provider-evidence','mock-only');providers.setAttribute('data-v5-ai-evidence','guardrailed')}
    const validation=slides[14];if(validation){const lead=validation.querySelector('.fm-next-story-lead');if(lead)lead.textContent='자동 release gate는 connected-ai와 provider/browser fallback, bounded timeout, reload restoration, deterministic ranking ownership, Closed Beta Auth·join/cancel·operator recovery, account deletion boundary, audit traceability, responsive 320/375/390/430, axe와 exact Production smoke를 함께 확인합니다. 물리 기기와 수동 접근성 점검은 별도 manual QA로 남깁니다.';const story=validation.querySelector('.fm-next-story');if(story){if(validation.getAttribute('aria-hidden')==='false')story.setAttribute('tabindex','0');else{story.dataset.fmCaseStudyTabindex='0';story.setAttribute('tabindex','-1')}}validation.setAttribute('data-v5-validation-evidence','acceptance')}

...
(function(){
  ...
  function patch(){
    ...
    const providers=slides[13];if(providers){const lead=providers.querySelector('.fm-next-story-lead');if(lead)lead.textContent='/app의 auth/payment/capacity/notification provider는 deterministic mock을 유지합니다. 반면 /beta는 Supabase Auth·Postgres·RLS·atomic RPC로 실제 회원·경기·포지션 정원·참가/취소를 연결하고, DB-backed in-app notification과 Supabase Edge Function + Resend transactional email을 사용합니다. 참가 확정·사용자 취소 메일은 실제 Production delivery까지 검증했으며 실제 PG·push notification·external analytics는 아직 연결하지 않았습니다.';providers.setAttribute('data-v5-provider-evidence','mock-only');providers.setAttribute('data-v5-ai-evidence','guardrailed')}
    const validation=slides[14];if(validation){const lead=validation.querySelector('.fm-next-story-lead');if(lead)lead.textContent='자동 release gate는 connected-ai와 provider/browser fallback, bounded timeout, reload restoration, deterministic ranking ownership, Closed Beta Auth·join/cancel·operator recovery, account deletion boundary, audit traceability, transactional email contract, responsive 320/375/390/430, axe와 exact Production smoke를 함께 확인합니다. 참가 확정·사용자 취소 메일은 실제 사용자 동작으로 Resend delivered까지 별도 검증했고, 물리 기기와 수동 접근성 점검은 manual QA로 남깁니다.';const story=validation.querySelector('.fm-next-story');if(story){if(validation.getAttribute('aria-hidden')==='false')story.setAttribute('tabindex','0');else{story.dataset.fmCaseStudyTabindex='0';story.setAttribute('tabindex','-1')}}validation.setAttribute('data-v5-validation-evidence','acceptance')}
    const outcome=slides[15];if(outcome){const lead=outcome.querySelector('.fm-next-story-lead');if(lead)lead.textContent='현재 제품은 /app의 sample·mock 경계와 /beta의 실제 Supabase 참가 경계를 분리했습니다. AI는 검색 조건 해석에 한정되고 /app의 순위·추천 이유는 deterministic recommendation이 소유합니다. Closed Beta는 실제 회원·경기·정원·참가/취소·계정 삭제와 참가 이벤트 transactional email까지 연결하되 결제와 push notification은 연결하지 않습니다.'}

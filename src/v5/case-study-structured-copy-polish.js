/* FootMate Case Study · structured copy grammar polish.
   Structured tables, flows, cards, states, criteria, reasons and trade-offs use compact phrase grammar.
...
(function(){
  ...
  function patch(){
    ...
    setRow(slides[1],'검증 범위','Beta 가설 확인 · 사용자 조사·경쟁사 우위 미입증');

    // 03 · Persona · JTBD — assumptions, tasks and observations share phrase grammar.
    ...
    if(persona[2])setLines(persona[2],'b',['교육생 6명 · iOS 4 / Android 2 · 과업 분산']);
    ...
    setCardLines(principles,0,['판단 기준 통합 · 선택 맥락 보존','무료 Beta · 인증 · 정원 · 참가 · 취소 · 체크인 · 복구 검증']);
    setCardLines(principles,1,['대기열 · 알림 · 경기 후 피드백 → 자리 회복 · 재탐색','참가 전환 · 반복 이용 효과 → 실제 이용 데이터 검증 과제']);
    setCardLines(principles,2,['실제 PG 유보 · 수익화 검증 제외','AI 자동 참가 제외 · 사용자 최종 확인(HITL)']);

    // 05 · Guest First — decision, reason and trade-off are compact decision facts.
    ...
    setRow(slides[7],'상태 보존','로그인 전 선택 경기 · 복귀 위치 유지');
    ...
    setRow(slides[10],'개인 프로젝트','협의 가능한 설계 범위 · 실제 다인 협업 성과 아님');

    // 12 · KPI & Validation — all QA cards use compact evidence phrases.
    const qaCards=slides[11].querySelectorAll('.fm-next-cs-grid.three .fm-next-cs-card');
    if(qaCards[1])setLines(qaCards[1],'p',['실제 OAuth 로그인 · 이메일 최종 전달','Web Push · 브라우저·OS 표시','사용자 만족도·전환 성과와 별도']);
    if(qaCards[2])setLines(qaCards[2],'p',['중복 · 용어 · 구현-설명 불일치','자동 QA·사람 검수 PASS 판정 대체 아님']);

    // 13 · Release & Learnings — implementation boundary, task scope and next steps stay phrase-like.
    ...
    setRow(slides[12],'학습·다음 단계','구체 행동 과업 → 동선별 버그·막힘 재검증 · 제품·운영 상태 고도화 → Production QA → KPI 측정 준비 · 실제 이용자 KPI → Baseline부터 측정');

/* FootMate Case Study · structured copy grammar polish.
   Inside 02–13 tables, flows, cards and evidence rows, use short phrase grammar without periods.
   Longer explanatory sentences belong outside structured components. */
(function(){
  let applied=false;
  ...
  function patch(){
    ...
    // 02 · Problem & Goal — all structured values stay scannable phrase facts.
    ...
    setRow(slides[1],'검증 범위','설계 가설 · Beta 검증 대상 · 사용자 조사·경쟁사 우위 미입증');

    // 03 · Persona · JTBD — assumption, task and observation stay compact and parallel.
    ...
    setLines(slides[2],'.fm-next-cs-jtbd p',[
      '가설 · 적합 이유 빠른 확인',
      '과업 · 회원가입 전 · Kakao · Google · 이메일',
      '관찰 · 동선별 버그 · 막힘 · 요구사항 조정'
    ]);

    // 04 · Scope & Priority — priority cards use phrase grammar throughout.
    const principles=slides[3].querySelector('.fm-next-cs-principles');
    setCardLines(principles,0,['판단 기준 통합 · 선택 맥락 보존','Beta 검증 · 인증 · 정원 · 참가 · 취소 · 체크인 · 복구']);
    setCardLines(principles,1,['대기열 · 알림 · 경기 후 피드백','자리 회복 · 재탐색 · 실제 이용 데이터 검증 과제']);
    setCardLines(principles,2,['실제 PG 유보 · 수익화 검증 제외','AI 자동 참가 제외 · 사용자 최종 확인(HITL)']);

    // 05 · Guest First — decision, reason and trade-off are compact structured values.
    ...
    // 06 · Recommendation — decision, quality criterion and trade-off share phrase grammar.
    setRow(slides[5],'결정','최근 선호 · 추천 보조 입력');
    setRow(slides[5],'품질 기준','후보 · 순위 · 이유 · 결정론적 추천 엔진 소유');
    setRow(slides[5],'Trade-off','현재 의도 불일치 가능 · 조건 수정 · 재탐색 허용');

    // 07 · Decision Detail — actions and trade-off are compact structured values.
    setRow(slides[6],'핵심 행동','참가하기');
    setRow(slides[6],'보조 행동','저장 · 최대 2경기 비교');
    setRow(slides[6],'Trade-off','최대 2경기 비교 · 참가 전 취소·환불 기준 확인');
    ...
    setRow(slides[7],'상태 보존','로그인 전 선택 경기 · 복귀 위치 유지 · 반복 선택 방지');
    ...
    setRow(slides[10],'실제 연결','Vercel AI Gateway · Supabase · Resend · Web Push · Storage 실연동');
    setRow(slides[10],'미연동','실제 PG · 외부 분석 도구 미연동');
    setRow(slides[10],'정의한 기준','API·데이터·권한·오류·재시도 · IA·상태별 화면·CTA · 취소·정원·복구 정책');

    // 12 · KPI & Validation — QA evidence cards use phrase grammar throughout.
    const qaCards=slides[11].querySelectorAll('.fm-next-cs-grid.three .fm-next-cs-card');
    if(qaCards[1])setLines(qaCards[1],'p',['실제 OAuth 로그인 · 이메일 최종 전달','Web Push 브라우저 · OS 표시','사용자 만족도 · 전환 성과 검증 제외']);
    if(qaCards[2])setLines(qaCards[2],'p',['중복 · 용어 · 구현-설명 불일치 검토','자동 QA · 사람 검수 PASS 판정 대체 아님']);

    // 13 · Release & Learnings — environment, task scope and next steps stay phrase-style.
    const outcomes=slides[12].querySelectorAll('.fm-next-cs-outcomes>div');
    if(outcomes[0])setLines(outcomes[0],'p',['AI Gateway 실연동 · 결정론적 추천','샘플 경기 데이터 · 인증·결제·정원·알림 시뮬레이션']);
    if(outcomes[1])setLines(outcomes[1],'p',['Supabase 인증·경기·정원·참가/취소·체크인 실연동','Google/Kakao OAuth · 이메일 · Web Push · 미디어 실환경 검증']);
    if(outcomes[2])setLines(outcomes[2],'p',['실제 PG · 외부 분석 도구 미연동','수익성 · 실제 이용 지표 미검증']);

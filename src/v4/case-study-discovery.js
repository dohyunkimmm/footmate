...
(function(){
  ...
  function patch(){
    ...
    const recommendationReady=document.documentElement.dataset.footmateCaseStudyRelease==='4.1.0';
    if(slides.length!==16||!note||!slides[4]?.querySelector('.fm-next-story')||!recommendationReady)return false;

    note.innerHTML='v4.2.0 · Discovery & Search<br>Matchday Companion';
    ...
    const recoveryLead=recovery.querySelector('.fm-next-story-lead');
    if(recoveryLead)recoveryLead.textContent='검색 결과가 0개가 되어도 막다른 화면으로 두지 않습니다. 조건 넓히기와 전체 조건 해제를 제공하고, 선택한 검색 조건은 URL과 세션에 남겨 다시 돌아와도 같은 탐색 맥락을 유지합니다.';

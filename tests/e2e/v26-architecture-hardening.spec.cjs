...
async function boot(page,width=375,height=812){
  ...
  await page.waitForFunction(()=>window.__footmateV2===true&&!!window.FootMateV28&&!!window.FootMateV27&&!!window.FootMateV26&&!!window.FootMateV25);
  return failures;
}
function expectNoFailures(failures){expect(failures,failures.join('\n')).toEqual([])}

test('v2.6 architecture boundaries remain available under v2.8',async({page})=>{
  ...
  const snapshot=await page.evaluate(()=>({
    ...
    previousArchitecture:window.FootMateV2Runtime?.previousReleaseArchitecture,
  ...
  expect(snapshot.v25).toMatchObject({version:'2.5.0',currentReleaseVersion:'2.8.0',compatibility:true});
  expect(snapshot.availability).toMatchObject({architecture:'v2.6-availability-verification-boundary',freshness:{source:'prototype-session',realtime:false,serverVerified:false}});
  expectNoFailures(failures);
});

test('v2.6 decision trace persistence remains durable under v2.8',async({page})=>{
  const failures=await boot(page);
  const trace=await page.evaluate(()=>{
    window.FootMateV2Runtime.productStore.set({creditBalance:3000},'e2e-v28-trace');

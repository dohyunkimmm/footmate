...
async function boot(page,width=375,height=812){
  ...
  await page.waitForFunction(()=>window.__footmateV2===true&&!!window.FootMateV27&&!!window.FootMateV26&&!!window.FootMateV25);
  return failures;
}
function expectNoFailures(failures){expect(failures,failures.join('\n')).toEqual([])}

test('v2.6 architecture boundaries remain available under v2.7',async({page})=>{
  ...
  expect(snapshot.v25).toMatchObject({version:'2.5.0',currentReleaseVersion:'2.7.0',compatibility:true});
...
test('v2.6 decision trace persistence remains durable under v2.7',async({page})=>{
  const failures=await boot(page);
  const trace=await page.evaluate(()=>{
    window.FootMateV2Runtime.productStore.set({creditBalance:3000},'e2e-v27-trace');

...
test('v4.3 compare intent caps at two matches',async({page})=>{
  const errs=await detail(page);
  await compareButton(page).click();
  await switchMatch(page,'gwanggyo-2130');await compareButton(page).click();
  await switchMatch(page,'yeongtong-1900');await compareButton(page).click();
...
test('v4.3 decision detail stays mobile-safe and accessible',async({page})=>{
  ...
  await compareButton(page).click();await switchMatch(page,'gwanggyo-2130');await compareButton(page).click();await page.getByRole('button',{name:'비교하기',exact:true}).click();

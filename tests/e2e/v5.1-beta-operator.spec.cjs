...
async function mockOperator(page,{allowed=true,empty=false}={}){
  ...
  let participants=empty?[]:[{id:'p-1',match_id:matchId,user_id:participantId,position:'MF',status:'confirmed',joined_at:'2099-01-01T00:00:00Z',canceled_at:null,checked_in_at:null,created_at:'2099-01-01T00:00:00Z',updated_at:'2099-01-01T00:00:00Z'}];
  ...
  await page.route(`${origin}/**`,async route=>{
    ...
    if(url.pathname==='/rest/v1/rpc/operator_check_in_participant'){
      const body=JSON.parse(request.postData()||'{}');participants=participants.map(item=>item.user_id===body.p_user_id?{...item,checked_in_at:'2099-09-25T19:31:00+09:00'}:item);return fulfill(route,[{participation_id:'p-1',match_id:body.p_match_id,user_id:body.p_user_id,checked_in_at:'2099-09-25T19:31:00+09:00'}]);
    }
    if(url.pathname==='/rest/v1/rpc/operator_save_match'||url.pathname==='/rest/v1/rpc/operator_save_match_v2'){
      const body=JSON.parse(request.postData()||'{}');const id=body.p_match_id||newMatchId;const existing=matches.find(item=>item.id===id);const slots=(body.p_slots||[]).filter(slot=>Number(slot.capacity_total)>0).map(slot=>({position:slot.position,capacity_total:Number(slot.capacity_total),joined_count:existing?.match_slots?.find(current=>current.position===slot.position)?.joined_count||0,remaining_spots:Number(slot.capacity_total)-(existing?.match_slots?.find(current=>current.position===slot.position)?.joined_count||0)}));const joined=slots.reduce((sum,slot)=>sum+slot.joined_count,0);const row={id,title:body.p_title,venue_name:body.p_venue_name,area_label:body.p_area_label,address:body.p_address,region:body.p_region,level:body.p_level,starts_at:body.p_starts_at,cancel_cutoff_at:body.p_cancel_cutoff_at||existing?.cancel_cutoff_at||null,check_in_opens_at:body.p_check_in_opens_at||existing?.check_in_opens_at||null,price_krw:0,capacity_total:body.p_capacity_total,joined_count:joined,remaining_spots:body.p_capacity_total-joined,format_label:body.p_format_label,surface:body.p_surface,duration_minutes:body.p_duration_minutes,status:body.p_status==='open'&&joined>=body.p_capacity_total?'full':body.p_status,created_by:userId,created_at:'2099-01-01T00:00:00Z',updated_at:'2099-01-01T00:00:00Z',match_slots:slots};matches=[row,...matches.filter(item=>item.id!==id)];return fulfill(route,[{match_id:id,match_status:row.status}]);
    }
    ...
    if(url.pathname==='/rest/v1/rpc/operator_complete_match'){
      const body=JSON.parse(request.postData()||'{}');matches=matches.map(item=>item.id===body.p_match_id?{...item,status:'completed'}:item);return fulfill(route,[{match_id:body.p_match_id,match_status:'completed',checked_in_participants:0}]);
    }
...
test('operator creates, edits and cancels real match operations with participant recovery',async({page})=>{
  ...
  expect(backend.getMatches().some(item=>item.id===newMatchId&&item.status==='open'&&item.cancel_cutoff_at&&item.check_in_opens_at)).toBe(true);

  await page.getByRole('button',{name:/새 Closed Beta 경기/}).click();
  page.once('dialog',dialog=>dialog.accept());

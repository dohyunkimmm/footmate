...
test('v2.4 core funnel architecture remains available under v2.8',async({page})=>{
  ...
  expect(snapshot.release).toMatchObject({version:'2.4.0',currentReleaseVersion:'2.8.0',previousReleaseVersion:'2.3.0',schemaVersion:'2.1.0',coreFunnelExperience:'v2.4-core-funnel-experience',componentSource:'src/v2/demo/core-funnel-components.js',cssOwnership:'src/v2/styles/core-funnel.css',compatibility:true});
  expect(snapshot.v23).toMatchObject({version:'2.3.0',currentReleaseVersion:'2.8.0',compatibility:true});
  expect(snapshot.stylePaths).toContain('/src/v2/styles/core-funnel.css');
  expect(snapshot.stylePaths).toContain('/src/v2/styles/visual-experience.css');
  expect(snapshot.stylePaths).toContain('/src/v2/styles/visual-identity.css');
  expectNoFailures(failures);
});

test('v2.4 home → filter → results → detail → payment funnel remains usable under v2.8',async({page})=>{
  const failures=await boot(page);
...
test('v2.4 core funnel remains usable at 320px under v2.8',async({page})=>{

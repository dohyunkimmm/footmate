...
test('Case Study assigns one distinct responsibility to each product-story section',async({page})=>{
  ...
  expect(navTitles).toEqual([
    ...
    '로그인','참가 · 결제','경기 당일 · 재탐색','복구','Domain Architecture','Provider · AI 경계','검증','Production 범위'
  ]);
  ...
  for(const lead of normalized)expect(lead.length).toBeLessThanOrEqual(460);
});
...
test('Case Study removes repeated decisions and gives each later section one clear job',async({page})=>{
  ...
  expect(signIn).toContain('/app: mock 인증 · /beta: Supabase Auth');
  ...
  expect(provider).toContain('실제 PG와 외부 analytics는 미연동');
  ...
  expect(production).toContain('실제 PG · 외부 analytics');
...
test('reader-facing copy avoids unnecessary English while preserving real technical terms',async({page})=>{
  await openCaseStudy(page);

  const navigation=await page.locator('.toc').innerText();
  for(const unnecessary of ['Overview','Problem','Product Thesis','Core Journey','Decision 01','Sign in','Join · Payment','Recovery','Validation','Production Boundary']){

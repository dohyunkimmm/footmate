...
test('02–13 structured cards, flows and values use compact phrase grammar',async({page})=>{
  ...
  expect(await rowValue(visibleSlide(page,1),'검증 범위')).toBe('Beta 가설 확인 · 사용자 조사·경쟁사 우위 미입증');

  const personaValues=await visibleSlide(page,2).locator('.fm-next-cs-persona>div b').allTextContents();
  expect(personaValues.map(value=>value.trim().replace(/\s+/g,' '))).toEqual([
    '평일 저녁 · 주 1~2회 · 30분 안쪽 이동',
    '시간 · 거리 · 레벨 · 포지션 · 남은 자리',
    '교육생 6명 · iOS 4 / Android 2 · 과업 분산'
  ]);
  const jtbdLines=await visibleSlide(page,2).locator('.fm-next-cs-jtbd p .fm-cs-line').allTextContents();
  expect(jtbdLines).toEqual([
    '가설 · 오늘 뛸 경기의 적합 이유 빠른 이해',
    '과업 · 가입 전 · Kakao · Google · 이메일',
  ...
  expect(priorityLines).toEqual([
    '판단 기준 통합 · 선택 맥락 보존','무료 Beta · 인증 · 정원 · 참가 · 취소 · 체크인 · 복구 검증',
    '대기열 · 알림 · 경기 후 피드백 → 자리 회복 · 재탐색','참가 전환 · 반복 이용 효과 → 실제 이용 데이터 검증 과제',
  ...
  expect(await rowValue(visibleSlide(page,7),'상태 보존')).toBe('로그인 전 선택 경기 · 복귀 위치 유지');
  ...
  expect((await qaCards.nth(1).locator('p').innerText()).trim().replace(/\s+/g,' ')).toBe('실제 OAuth 로그인 · 이메일 최종 전달 Web Push · 브라우저·OS 표시 사용자 만족도·전환 성과와 별도');
  expect((await qaCards.nth(2).locator('p').innerText()).trim().replace(/\s+/g,' ')).toBe('중복 · 용어 · 구현-설명 불일치 자동 QA·사람 검수 PASS 판정 대체 아님');
  ...
  expect(outcomes).toEqual([
    ...
    'Supabase 인증 · 경기 · 정원 · 참가/취소 · 체크인 실연동','Google/Kakao OAuth · 이메일 · Web Push · 미디어 실환경 검증',
  ...
  expect(await rowValue(visibleSlide(page,12),'학습·다음 단계')).toContain('구체 행동 과업 → 동선별 버그·막힘 재검증');
});

test('02–13 structured tables, flows and cards do not fall back to sentence copy',async({page})=>{
  await openCaseStudy(page);
  const selector=[
    '.fm-next-cs-card p','.fm-next-cs-persona b','.fm-next-cs-jtbd p','.fm-next-cs-before-after p',
    '.fm-next-cs-stack p','.fm-next-cs-auth-flow b','.fm-next-cs-detail-order span','.fm-next-cs-modes p',
    '.fm-next-cs-day-states p','.fm-next-cs-recovery span','.fm-next-cs-outcomes p','.fm-cs-reasons dd'
  ].join(',');
  const values=await page.locator('.slide:not([hidden])').locator(selector).allTextContents();
  const sentenceEnding=/(?:합니다|했습니다|됩니다|않습니다|있습니다|없습니다|입니다)[.!?。]?\s*$/;
  expect(values.length).toBeGreaterThan(45);
  expect(values.filter(hasSentencePunctuation)).toEqual([]);
  expect(values.map(value=>value.trim()).filter(value=>sentenceEnding.test(value))).toEqual([]);
});

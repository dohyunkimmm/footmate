...
test('Home is the AI Match Assistant entry with one compact For You list',async({page})=>{
  ...
  await expect(screen.locator(':scope > .fm-next-greeting')).toHaveCount(0);
  await expect(screen.locator(':scope > .fm-next-topbar [data-action="nav-profile"]')).toHaveCount(0);
  await expect(screen.locator('.fm-next-nav [data-action="nav-profile"]')).toBeVisible();

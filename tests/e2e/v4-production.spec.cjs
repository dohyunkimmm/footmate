...
test('v4.3 exact Production app, recommendation, discovery, decision and Case Study render',async({page})=>{
  ...
  await expect(page.locator('meta[name="footmate-release"]')).toHaveAttribute('content','4.3.0');
  ...
  await page.waitForFunction(()=>document.querySelectorAll('.slide').length===16&&document.documentElement.dataset.footmateCaseStudyRelease==='4.3.0');
  await expect(page.locator('.fm-next-cover-note')).toContainText('v4.3.0');await expect(page.locator('.fm-next-cover-frame iframe')).toHaveAttribute('src','/app?embed=1');
...
test('v4.3 exact Production Case Study stays mobile-safe across all 16 sections',async({page})=>{const errors=capture(page);await page.setViewportSize({width:390,height:844});await page.goto('/',{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>document.querySelectorAll('.slide').length===16&&document.documentElement.dataset.footmateCaseStudyRelease==='4.3.0');for(let index=0;index<16;index+=1){await page.evaluate(i=>window.goTo(i),index);await page.waitForTimeout(20);const overflow=await page.evaluate(()=>document.documentElement.scrollWidth-document.documentElement.clientWidth);expect(overflow,`Production Case Study horizontal overflow on slide ${index+1}`).toBeLessThanOrEqual(1)}expect(errors).toEqual([])});

test('v4.3 exact Production compatibility aliases stay on the current product',async({page})=>{for(const route of ['/demo','/next']){await page.goto(route,{waitUntil:'domcontentloaded'});await expect(page.locator('meta[name="footmate-release"]')).toHaveAttribute('content','4.3.0');await expect(page.locator('#footmate-next')).toBeVisible()}});

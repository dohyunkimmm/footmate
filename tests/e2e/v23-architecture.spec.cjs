...
async function boot(page){
  ...
  await page.waitForFunction(()=>window.__footmateV2===true&&!!window.FootMateV27&&!!window.FootMateV26&&!!window.FootMateV25&&!!window.FootMateV24&&!!window.FootMateV23);
  return failures;
}
function expectNoFailures(failures){expect(failures,failures.join('\n')).toEqual([])}

test('v2.3 release contract remains available as a compatibility alias under v2.7',async({page})=>{
  ...
  expect(snapshot.runtimeVersion).toBe('2.7.0');
  expect(snapshot.releaseDataset).toBe('2.7');
  expect(snapshot.releaseArchitecture).toBe('v2.7-visual-experience');
  expect(snapshot.v23).toMatchObject({version:'2.3.0',currentReleaseVersion:'2.7.0',previousReleaseVersion:'2.2.0',schemaVersion:'2.1.0',scenarioPersistence:'v2.3-scenario-persistence-migration',scenarioPresentation:'v2.3-scenario-presenter',architecture:'v2.3-compatibility-boundary-reduction',compatibility:true});
  expect(snapshot.v22).toMatchObject({version:'2.2.0',currentReleaseVersion:'2.7.0',compatibility:true});
  expectNoFailures(failures);
});

test('v2.3 canonical scenario persistence still restores under v2.7',async({page})=>{
  const failures=await boot(page);
  await page.evaluate(()=>{

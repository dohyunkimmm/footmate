...
async function boot(page){
  ...
  await page.waitForFunction(()=>window.__footmateV2===true&&!!window.FootMateV24&&!!window.FootMateV23);
  return failures;
}
function expectNoFailures(failures){expect(failures,failures.join('\n')).toEqual([])}

test('v2.3 release contract remains available as a compatibility alias',async({page})=>{
  ...
  expect(snapshot.runtimeVersion).toBe('2.4.0');
  expect(snapshot.releaseDataset).toBe('2.4');
  expect(snapshot.releaseArchitecture).toBe('v2.4-core-funnel-experience');
  expect(snapshot.v23).toMatchObject({
    version:'2.3.0',
  ...
  expect(snapshot.v22).toMatchObject({version:'2.2.0',currentReleaseVersion:'2.4.0',compatibility:true});
  expectNoFailures(failures);
});

test('v2.3 canonical scenario persistence still restores under v2.4',async({page})=>{
  const failures=await boot(page);
  await page.evaluate(()=>{

...
async function boot(page,width=375,height=812){
  ...
  await page.waitForFunction(()=>window.__footmateV2===true&&!!window.FootMateV28&&!!window.FootMateV27&&!!window.FootMateV26&&!!window.FootMateV25&&!!window.FootMateV24);
  return failures;
}
function expectNoFailures(failures){expect(failures,failures.join('\n')).toEqual([])}

test('v2.5 decision/recovery runtime remains compatible under v2.8',async({page})=>{
  ...
  expect(snapshot.version).toBe('2.8.0');
  expect(snapshot.architecture).toBe('v2.8-visual-identity');
  expect(snapshot.dataset).toBe('2.8');
  expect(snapshot.screens).toBe(39);
  expect(snapshot.release).toMatchObject({version:'2.5.0',currentReleaseVersion:'2.8.0',previousReleaseVersion:'2.4.0',schemaVersion:'2.1.0',decisionEngine:'v2.5-decision-recovery-engine',decisionRecoveryExperience:'v2.5-decision-recovery-experience',traceReplay:true,compatibility:true});
  expect(snapshot.v24).toMatchObject({version:'2.4.0',currentReleaseVersion:'2.8.0',compatibility:true});
...
test('v2.5 connects next-action, comparison, preflight and payment guard IX under v2.8',async({page})=>{
  const failures=await boot(page);
...
test('v2.5 blocks low-credit payment and offers inline recovery under v2.8',async({page})=>{
  const failures=await boot(page);
  await page.evaluate(()=>{
...
test('v2.5 converts a full match into waitlist recovery instead of payment under v2.8',async({page})=>{
  const failures=await boot(page);
  await page.evaluate(()=>{
...
test('v2.5 decision surfaces remain usable at 320px under v2.8',async({page})=>{

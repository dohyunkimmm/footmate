...
test('production browser smoke keeps compatibility and exact current-release verification separate', () => {
  assert.ok(source.includes("if (strictProduction) await page.waitForFunction(() => !!window.FootMateV26 && !!window.FootMateV25);"));
  assert.ok(source.includes("window.FootMateV26?.version || window.FootMateV25?.version || window.FootMateV24?.version || window.FootMateV23?.version || window.FootMateV22?.version || null"));
  ...
  assert.ok(source.includes("expect(release.v26Version).toBe('2.6.0')"));

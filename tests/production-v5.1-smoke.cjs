...
async function get(path){const response=await fetch(base+path,{redirect:'follow',headers:{'user-agent':'FootMate-v5.1.1-QA'}});a(response.ok,`${path} ${response.status}`);return response}
(async()=>{
  if(!strict){console.log('SKIP v5.1.1 exact Production HTTP smoke');return}
  ...
  a(app.includes('footmate-release" content="5.1.1"'),'app release');a(app.includes('/src/v5/ai-match-assistant.js?v=511'),'AI runtime');a(app.includes('/src/v5/ai-match-assistant.css?v=511'),'AI styles');a(cs.includes('footmate-case-study-release" content="5.1.1"'),'case release');a(cs.includes('/src/v5/case-study-connected.js?v=511'),'case narrative');a(beta.includes('FootMate | Closed Beta'),'beta route');a(beta.includes('/src/v5/beta.js?v=1'),'beta runtime');a(beta.includes('/src/v5/beta-push.js?v=1'),'beta push runtime');a(beta.includes('/src/v5/beta-media.js?v=1'),'beta media runtime');a(operator.includes('FootMate | Closed Beta Operator'),'operator route');a(operator.includes('/src/v5/beta-operator-mfa.js?v=1'),'operator MFA gate');a(operator.includes('/src/v5/beta-media.js?v=1'),'operator media runtime');a(serviceWorker.includes("addEventListener('push'"),'beta push service worker');a(betaConfig.connected===true,'beta backend connected');a(journey.includes("CONNECTED_PLATFORM_VERSION='5.1.1'"),'journey version');a(ai.includes("rankingOwner:'deterministic recommendation engine'"),'deterministic ranking ownership');a(api.version==='5.1.1','AI API version');a(api.provider==='vercel-ai-gateway','AI API provider');a(api.model==='inclusionai/ling-3.0-flash-vl-free','AI primary model');a(api.fallbackModel==='inclusionai/ling-3.0-flash-fin-free','AI fallback model');a(api.gatewayTimeoutMs===3000,'AI server timeout');a(typeof api.configured==='boolean','AI API configuration disclosure');for(const route of ['/demo','/next'])a((await(await get(route)).text()).includes('footmate-release" content="5.1.1"'),`${route} release`);
  ...
  console.log(`PASS v5.1.1 exact Production HTTP smoke (AI configured=${api.configured}, beta connected=${betaConfig.connected})`);

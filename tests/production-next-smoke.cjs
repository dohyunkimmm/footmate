...
async function main(){
  ...
  const checks=[];
  await check('next-entry','/next',({body,contentType})=>{assert(contentType.includes('text/html'),'/next must return HTML');assert(body.includes('/src/next/app.css'),'/next app stylesheet missing');assert(body.includes('/src/next/app.js'),'/next app module missing')},checks);

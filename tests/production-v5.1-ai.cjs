...
(async()=>{
  if(!strict){console.log('SKIP v5.5.1 exact Production AI inference');return}
  const response=await fetch(base+'/api/ai-match-assistant',{method:'POST',headers:{'content-type':'application/json','origin':base,'user-agent':'FootMate-v5.5.1-AI-QA'},body:JSON.stringify({message:'수원 인계에서 8시 이후 가까운 초중급 MF 경기 찾아줘',preferences:{region:'수원 · 영통',position:'MF',level:'중급'}})});
  const payload=await response.json().catch(()=>({}));
  assert(response.ok,`AI inference HTTP ${response.status}: ${JSON.stringify(payload)}`);
  assert(payload.version==='5.5.1','AI inference release');
  ...
  console.log(`PASS v5.5.1 exact Production AI inference (${payload.model}, fallbackUsed=${payload.fallbackUsed})`);

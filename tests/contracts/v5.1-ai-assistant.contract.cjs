...
(async()=>{
  ...
  try{
    ...
    assert.equal(health.statusCode,200);assert.equal(health.body.version,'5.5.1');assert.equal(health.body.provider,'vercel-ai-gateway');assert.equal(health.body.model,'inclusionai/ling-3.0-flash-vl-free');assert.equal(health.body.fallbackModel,'inclusionai/ling-3.0-flash-fin-free');assert.equal(health.body.gatewayTimeoutMs,3000);assert.equal(health.body.reasoningEffort,'none');assert.equal(health.body.configured,true);

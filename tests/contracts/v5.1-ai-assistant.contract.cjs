...
(async()=>{
  ...
  try{
    ...
    global.fetch=async(url,options)=>{
      ...
      const payload=JSON.parse(options.body);assert.equal(payload.model,'inclusionai/ling-3.0-flash-vl-free');assert.equal(payload.text,undefined);
    ...
    let calls=0;
    global.fetch=async(_url,options)=>{calls+=1;const payload=JSON.parse(options.body);if(calls===1){assert.equal(payload.model,'inclusionai/ling-3.0-flash-vl-free');return{ok:false,status:403,json:async()=>({error:{type:'no_providers_available',message:'No providers available'}})}}assert.equal(payload.model,'inclusionai/ling-3.0-flash-fin-free');return{ok:true,status:200,json:async()=>({output_text:'```json\n{"intent":"search","region":"수원 · 인계","position":"MF","level":"초중급","maxPrice":18000,"maxDistanceMin":20,"afterTime":"20:00","reply":"provider fallback으로 조건을 해석했어요."}\n```'})}};
    ...
    assert.equal(health.statusCode,200);assert.equal(health.body.version,'5.5.1');assert.equal(health.body.provider,'vercel-ai-gateway');assert.equal(health.body.model,'inclusionai/ling-3.0-flash-vl-free');assert.equal(health.body.fallbackModel,'inclusionai/ling-3.0-flash-fin-free');assert.equal(health.body.gatewayTimeoutMs,3000);assert.equal(health.body.configured,true);
    console.log('PASS v5.5.1 AI assistant contract');

...
(async()=>{
  ...
  try{
    ...
    global.fetch=async(url,options)=>{
      ...
      const payload=JSON.parse(options.body);assert.equal(payload.model,'openai/gpt-5.4-mini');assert.deepEqual(payload.reasoning,{effort:'none'});assert.equal(payload.text?.format?.type,'json_schema');assert.equal(payload.text?.format?.strict,true);
    ...
    assert.equal(connected.statusCode,200);assert.equal(connected.body.version,'5.1.1');assert.equal(connected.body.mode,'connected-ai');assert.equal(connected.body.provider,'vercel-ai-gateway');assert.equal(connected.body.model,'openai/gpt-5.4-mini');assert.equal(connected.body.fallbackUsed,false);assert.equal(connected.body.result.region,'수원 · 인계');assert.equal(connected.body.result.position,'MF');assert.equal(connected.body.result.afterTime,'20:00');assert.equal(connected.body.result.maxDistanceMin,20);
    ...
    let calls=0;
    global.fetch=async(_url,options)=>{calls+=1;const payload=JSON.parse(options.body);assert.deepEqual(payload.reasoning,{effort:'none'});if(calls===1){assert.equal(payload.model,'openai/gpt-5.4-mini');return{ok:false,status:404,json:async()=>({error:{type:'model_not_found',message:'Model not found'}})}}assert.equal(payload.model,'openai/gpt-5.4-nano');assert.equal(payload.text?.format?.type,'json_schema');return{ok:true,status:200,json:async()=>({output_text:'```json\n{"intent":"search","region":"수원 · 인계","position":"MF","level":"초중급","maxPrice":18000,"maxDistanceMin":20,"afterTime":"20:00","reply":"provider fallback으로 조건을 해석했어요."}\n```'})}};
    const fallback=await invoke({method:'POST',body:{message:'인계에서 초중급 MF 경기',preferences:{}},ip:'10.0.1.6'});
    assert.equal(fallback.statusCode,200);assert.equal(fallback.body.mode,'connected-ai');assert.equal(fallback.body.model,'openai/gpt-5.4-nano');assert.equal(fallback.body.fallbackUsed,true);assert.equal(fallback.body.result.region,'수원 · 인계');assert.equal(calls,2);
    ...
    assert.equal(denied.statusCode,502);assert.equal(denied.body.error,'ai_gateway_error');assert.equal(denied.body.gatewayType,'access_denied');assert.equal(denied.body.model,'openai/gpt-5.4-nano');

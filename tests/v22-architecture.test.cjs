...
test('v2.5 release metadata preserves v2.2/v2.3/v2.4 compatibility and v2.1 storage/event contracts', () => {
  ...
  for(const event of ['v2.2','v2.3','v2.4','v2.5'])assert.ok(bootstrap.includes(`'footmate:${event}:ready'`));
  for(const alias of ['FootMateV22','FootMateV23','FootMateV24','FootMateV25'])assert.ok(bootstrap.includes(`window.${alias}`));
  assert.ok(hardening.includes("eventContractVersion:'2.1.0'"));
  assert.ok(caseStudy.includes('v2.5.0 Decision & Recovery Experience'));
});

test('v2.2 inspector CSS compatibility alias remains while v2.5 shell loads canonical ownership', () => {
  ...
  assert.ok(shell.includes('bootstrap.js?v=20260919-1'));
});
